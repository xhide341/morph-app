...
export const useRoom = (roomId?: string) => {
  ...
  const fetchRoom = async (roomId: string): Promise<RoomInfo | null> => {
    try {
      const response = await fetch(`/${process.env.VITE_API_URL}/room/${roomId}/info`);
  ...
  const fetchRoomUsers = async (roomId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/${process.env.VITE_API_URL}/room/${roomId}/users`);
  ...
  const createRoom = async (roomId: string) => {
    try {
      const response = await fetch(`/${process.env.VITE_API_URL}/room/create`, {
        method: "POST",
  ...
  const shareRoom = async (roomId: string) => {
    try {
      ...
      // store url in redis
      const response = await fetch(`/${process.env.VITE_API_URL}/room/${roomId}/url`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
